...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True,abort_early=True,
              batch_size=8,max_iter=1000,const=0.01,confidence=0.0,early_stop_iters=100, binary_search_steps=9,
...
def zoo_attack(model, dataloader, num_samples=1):
    ...
    # samples is the how many samples to take in total : for targeted, 1 means 
    # all 9 class target -> 9 total samples whereas for untargeted the original data 
    # sample is taken i.e. 1 sample only 
    
    # if targeted one image is copied 9 items with 9 label other than its own
    inputs, targets = generate_data(dataloader,targeted,samples=num_samples,start=6) 
