...
def get_preds(model, inputs, dataset_name, device, correct_class=None, batch_size=25, return_cpu=True):
...
def get_least_likely(model, inputs, dataset_name, device, batch_size=25, return_cpu=True):
    num_batches = int(math.ceil(inputs.size(0) / float(batch_size)))
    softmax = torch.nn.Softmax()
    all_preds, all_probs = None, None
    transform = trans.Normalize(IMAGENET_MEAN, IMAGENET_STD)
