...
from grad_utils import get_preds_grad
...
parser.add_argument('--num_runs', type=int, default=3, help='number of image samples')
parser.add_argument('--batch_size', type=int, default=3, help='batch size for parallel runs')
...
if args.model.startswith('inception'):
    image_size = 299
    testset = dset.ImageFolder(args.data_root, utils.INCEPTION_TRANSFORM)
else:
    image_size = 224
    testset = dset.ImageFolder(args.data_root, utils.IMAGENET_TRANSFORM)
attacker = SimBA(model, 'imagenet', image_size, device=device)
...
if os.path.isfile(batchfile):
    ...
    while preds.ne(labels).sum() > 0:
        ...
        for i in list(idx):
            images[i], _ = testset[i]
            
        preds[idx], _ = utils.get_preds(model, images[idx], 'imagenet', device=device, batch_size=args.batch_size)
        print(preds[idx])
...
for i in range(adv.shape[0]):
  img = transform(adv[i])
  images_att.append(img)
  # path = "save/attacked_images/" + str(i) + ".jpg"
#   img.save(path)
# img.show()
...
pred, probabilities = utils.get_preds(model, adv, 'imagenet', device=device, batch_size=3)
print("old", pred, probabilities)

get_preds_grad(model, adv, 'imagenet', device=device, image=images_att, batch_size=3, attack=True)
get_preds_grad(model, images, 'imagenet', device=device, image=images_org, batch_size=3, attack=False)
# print("new", pred, probabilities)